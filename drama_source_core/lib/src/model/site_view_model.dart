...
class SiteViewModel {
  ...
  homeContent() async {
    ...
    if (site.getType() == 3) {
      ...
      result.setList(Result.fromJson(Json.parse(homeContent)).getList());
      return result;
  ...
  Future<String> _fetchExt(Site site, Map<String, String> params, bool limit) async{
    ...
    Future<Result> _fetchPic(Site site, Result result) async {
        if (site.getType() > 2 || result.getList().isEmpty || result.getList()[0].getVodPic().length > 0)
            return result;
        List<String> ids = [];
        if (site.getCategories().isEmpty)
            for (Vod item in result.getList()) ids.add(item.getVodId());
        else for (Vod item in result.getList())
